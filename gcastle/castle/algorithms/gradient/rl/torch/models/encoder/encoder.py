...
class AttnHead(nn.Module):
    ...
    def __init__(self, config, hidden_dim, out_sz):
        ...
        if self.config.use_gpu:
            self._seq_fts = nn.Conv1d(in_channels=hidden_dim, out_channels=out_sz, kernel_size=1, bias=False).cuda(self.config.gpu_id)

            self._f_1 = nn.Conv1d(in_channels=out_sz, out_channels=1, kernel_size=1).cuda(self.config.gpu_id)
            self._f_2 = nn.Conv1d(in_channels=out_sz, out_channels=1, kernel_size=1).cuda(self.config.gpu_id)

            self._ret = nn.Conv1d(in_channels=hidden_dim, out_channels=out_sz, kernel_size=1).cuda(self.config.gpu_id)

            self._bias = nn.Parameter(torch.Tensor(1)).cuda(self.config.gpu_id)
...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self, config, input_dimension, num_units=None):
        ...
        # Normalize
        if self.config.use_gpu:
            self.bn_layer = nn.BatchNorm1d(input_dimension).cuda(self.config.gpu_id)
        else:
...
class FeedForward(nn.Module):
    ...
    def __init__(self, config, num_units=[2048, 512]):
        ...
        if self.config.use_gpu:
            ...
            self.bn_layer1 = nn.BatchNorm1d(num_units[1]).cuda(self.config.gpu_id)  # 传入通道数
        else:
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, config, is_train):
        ...
        if self.config.use_gpu:
            self.emb = nn.Parameter(torch.Tensor(*(1, self.input_dimension, self.input_embed))).cuda(self.config.gpu_id)
        else:
        ...
        # Batch Normalization
        if self.config.use_gpu:
            self.bn_layer2 = nn.BatchNorm1d(self.input_dimension).cuda(self.config.gpu_id)
        else:
    ...
    def forward(self, inputs):
        ...
        for i in range(self.num_stacks):  # num blocks
            ...
            self.enc = self.multihead_attention[i](self.enc, num_heads=self.num_heads, dropout_rate=0.0, is_training=self.is_training)
            # Feed Forward

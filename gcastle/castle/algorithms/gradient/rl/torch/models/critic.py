...
class Critic(nn.Module):
    ...
    def __init__(self, config, is_train):
        ...
        if self.config.use_gpu:
            layer0 = nn.Linear(in_features=self.input_dimension, out_features=self.num_neurons).cuda(self.config.gpu_id)
            torch.nn.init.xavier_uniform(layer0.weight)
            self.h0_layer = nn.Sequential(layer0, nn.ReLU()).cuda(self.config.gpu_id)

            self.layer1 = nn.Linear(in_features=self.num_neurons, out_features=1).cuda(self.config.gpu_id)
            torch.nn.init.xavier_uniform(self.layer1.weight)
            self.layer1.bias.data = torch.Tensor([self.init_baseline]).cuda(self.config.gpu_id)
        else:

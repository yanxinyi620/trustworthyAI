...
class BaseModel(nn.Cell):
    ...
    def get_matrix_exp(A):
        ...
        Returns
        -------
        E: matrix exponent value of A
        """
        E = msnp.zeros(A.shape, dtype=mstype.float32)
        F = msnp.eye(A.shape[0], dtype=mstype.float32)
        k = 1.0

        while msnp.norm(F, 1) > 0:
            E = E + F
            F = msnp.matmul(A, F) / k
            k += 1.0
        return E
    ...
    def forward_given_params(self, x, weights, biases):
        ...
        for k in range(self.hidden_num + 1):
            ...
            if k == 0:
                ...
                x = ops.matmul(weights[k], ops.expand_dims(x, 3)).squeeze(3) + biases[k]
